...
/**
 * OPT-IN acceptance check against the REAL, locally installed and authenticated Codex CLI.
 *
 * It is deliberately not part of any gate and never runs in CI: it starts one real agent turn, which
 * consumes the user's own Codex credits. It refuses to run without an explicit flag.
 *
 * What it does, and nothing more: create a throwaway workspace outside the repository, ask Codex for
...
const ENABLED = process.env.COMPAZIO_CODEX_LOCAL === "1";

if (!ENABLED) {
  process.stdout.write(
    [
      "Codex real-adapter check is DISABLED.",
      "",
      "This check starts ONE real Codex turn and therefore consumes your own Codex credits.",
      "It uses the Codex CLI you already installed and authenticated; Compazio never asks for,",
      "reads or stores a credential.",
      "",
      "To run it deliberately:",
      "  COMPAZIO_CODEX_LOCAL=1 pnpm test:adapter:codex-local",
      ""
    ].join("\n")
  );
  process.exit(0);
...
const EXPECTED = "compazio-codex-ok";
const TASK = [
...
async function main(): Promise<number> {
  process.stdout.write("Running ONE real Codex turn. This consumes your own Codex credits.\n\n");
  // A throwaway workspace, deliberately outside this repository so the real agent cannot reach it.
  const workspace = await mkdtemp(join(tmpdir(), "compazio-codex-real-"));
  ...
  try {
    const availability = await adapters.detect("codex");
    check(
    ...
    check("Codex published an official artifact", artifact !== null);
  ...
  process.stdout.write(`\nCODEX LOCAL CHECK: ${checks.length - failed.length}/${checks.length}\n`);
...
main()
  ...
  .catch((error: unknown) => {
    process.stderr.write(
      `Codex local check crashed: ${error instanceof Error ? error.message : String(error)}\n`
    );
    process.exit(1);
  });
