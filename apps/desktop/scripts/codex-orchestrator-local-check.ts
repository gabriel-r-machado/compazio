...
import { CodexOrchestratorPort } from "../src/main/codex-orchestrator-port";
...
/**
 * OPT-IN acceptance for CODEX AS A PLANNER against the REAL, locally installed and authenticated Codex
 * CLI. It is deliberately not part of any gate and never runs in CI: it starts ONE real Codex turn and
 ...
 * never installs, commits, pushes or deploys anything. The turn runs read-only inside a disposable
...
const GUARD = "COMPAZIO_CODEX_ORCHESTRATOR_LOCAL" as const;
...
if (process.env[GUARD] !== "1") {
  process.stdout.write(
    [
      "Codex planning check is DISABLED.",
      "",
      "This check starts ONE real Codex turn and therefore consumes your own provider credits.",
      "It uses the Codex CLI you already installed and authenticated; Compazio never asks for,",
      "reads or stores a credential. It only PLANS: nothing is materialized, started or executed.",
      "",
      "To run it deliberately:",
      `  ${GUARD}=1 pnpm test:orchestrator:codex-local`,
      ""
    ].join("\n")
  );
  process.exit(0);
...
async function main(): Promise<number> {
  process.stdout.write(
    "Running ONE real Codex planning turn. This consumes your own provider credits.\n\n"
  );
  // A throwaway workspace, outside this repository. Planning sees only a disposable snapshot of it.
  const workspace = await mkdtemp(join(tmpdir(), "compazio-codex-planner-"));
  await writeFile(
    join(workspace, "package.json"),
    `${JSON.stringify({ name: "codex-planner-check", private: true, scripts: { test: "node --test" } }, null, 2)}\n`,
    "utf8"
  );
  ...
  let realCalls = 0;
  const supervisorRunner = new SupervisorPlanningRunner(supervisor, "codex");
  const codexPort = new CodexOrchestratorPort({
    runner: {
      run: async (input) => {
        realCalls += 1;
        if (realCalls > 1) {
  ...
  const registry = new OrchestratorRegistry([codexPort]);
  ...
  const deadline = setTimeout(() => {
    process.stderr.write("The Codex planning check exceeded its global timeout.\n");
    process.exit(1);
  }, GLOBAL_TIMEOUT_MS);
  deadline.unref?.();
  ...
  try {
    const availability = await registry.detect("codex");
    check(
      "the real Codex CLI is installed and reports a version",
      availability.available,
      availability.issue?.message
    );
    check(
      "the planner was resolved through the OrchestratorRegistry",
      registry.has("codex") && registry.get("codex") === codexPort,
      registry.knownIds().join(",")
    );
    if (!availability.available) return 1;
    const port = registry.get("codex");
    process.stdout.write(`Codex version: ${availability.version ?? "unknown"}\n`);
    ...
    check(
      "Codex produced a schema-valid plan",
      result.plan !== null,
      result.diagnostics
    ...
    check("the plan was written by the planner that was asked", result.adapterId === "codex");
    check(
    ...
    check("exactly one real Codex turn was spent", realCalls === 1, String(realCalls));
...
`,
    ...
    check(
      "the provenance came back naming Codex, with its version and time",
      reloadedProvenance.success &&
        reloadedProvenance.data.adapter === "codex" &&
    ...
    check("reloading the plan called Codex zero more times", realCalls === 1, String(realCalls));
    check(
  ...
  process.stdout.write(
    `\nCODEX PLANNING CHECK: ${String(checks.length - failed.length)}/${String(checks.length)}\n`
  );
...
main()
  ...
  .catch((error: unknown) => {
    process.stderr.write(
      `Codex planning check crashed: ${error instanceof Error ? error.message : String(error)}\n`
    );
    process.exit(1);
  });
