...
import { CodexOrchestratorPort, type PlanningProcessRunner } from "./codex-orchestrator-port";
...
/**
 * The Codex planner over the real transport, against a deterministic fixture. No real Codex is ever
 * called here. The fixture speaks the real protocol — `exec`, `--sandbox read-only`,
 * `--output-last-message`, prompt on stdin, no positional — so a change that breaks the contract fails
 * here rather than later against the installed CLI.
 */

const here = dirname(fileURLToPath(import.meta.url));
const plannerFixture = resolve(here, "..", "..", "e2e", "fixtures", "codex-planner-fixture.mjs");
...
function fixtureRunner(supervisor: ProcessSupervisor): PlanningProcessRunner {
  const inner = new SupervisorPlanningRunner(supervisor, "codex");
  return {
    run: async (input) =>
      inner.run({
        ...input,
...
interface Harness {
  readonly port: CodexOrchestratorPort;
...
function compose(options: { readonly timeoutMs?: number } = {}): Harness {
  ...
  const port = new CodexOrchestratorPort({
...
describe("CodexOrchestratorPort (fixture only, never the real Codex)", () => {
  ...
  beforeAll(async () => {
    ...
    workspace = await mkdtemp(join(tmpdir(), "codex-planner-workspace-"));
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        // 10: planning is read-only; the fixture itself refuses anything else.
        expect(launch?.args).toContain("--sandbox");
        expect(launch?.args[(launch.args.indexOf("--sandbox") ?? 0) + 1]).toBe("read-only");
        expect(launch?.args).toContain("--output-last-message");
