#!/usr/bin/env node
// Controlled test double for the Codex CLI acting as a PLANNER. It is NEVER imported by production
// code, is excluded from the shipped bundle, and never calls the real Codex — it deterministically
// simulates `codex exec` in planning mode so the CodexOrchestratorPort can be exercised end to end by
// swapping only the executable.
//
// It speaks the REAL protocol the port targets, so a change that breaks the contract fails here
// instead of failing later against the real CLI:
//   * `--version` reports a version and exits 0;
//   * `exec` is required;
//   * `--sandbox read-only` is required — a planning turn must never be able to write the project;
//   * `--output-last-message <path>` is required, and the plan is written THERE, never to stdout;
//   * the prompt arrives EXCLUSIVELY on stdin, with NO positional message.
//
...
if (sandbox !== "read-only") {
  failLoudly(`planning must run read-only; received sandbox: ${sandbox ?? "(none)"}`);
}
if (argv.includes("--dangerously-bypass-approvals-and-sandbox")) {
  failLoudly("the planner must never bypass the sandbox");
}
const outputPath = readFlag("--output-last-message");
if (outputPath === null) {
  failLoudly("the planner must pass --output-last-message so the plan has a managed destination");
}
const workingDirectory = readFlag("--cd");
// A positional argument would mean the prompt reached the command line.
...
function failLoudly(reason) {
  process.stderr.write(`codex-planner-fixture: ${reason}\n`);
  process.exit(3);
}

/** A schema-valid three-node plan that spreads the work across all three executors. */
...
function run(mode) {
  switch (mode) {
    case "valid":
      ...
      break;
    case "smoke":
      // The plan the Electron smoke executes: three nodes, three different executors, and prompts that
      // the official executor fixtures understand. The planner still only PLANS — it runs nothing.
      writePlan({
        title: "Deliver the slice",
        summary: "Design it, build it, verify it.",
        nodes: [
    ...
    case "limit": {
      ...
      plan.nodes = Array.from({ length: 12 }, (_, index) =>
        node(`node-${index}`, `Step ${index}`, "implementer", "codex")
      );
      writePlan(plan);
      break;
    }
    ...
    case "huge": {
      ...
      writeRaw(outputPath, JSON.stringify(plan));
      break;
    }
...
function writeRaw(path, text) {
  ...
  if (path === outputPath) process.exit(0);
}
